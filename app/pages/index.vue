...
const upcoming = computed(() => [...events].filter(event => Date.parse(event.date) >= now.value).sort((a, b) => Date.parse(a.date) - Date.parse(b.date))[0])
...
<template>
  <div class="home-page">
    ...
    <section class="next-section wrap"><div class="section-heading"><div><div class="eyebrow"><span class="accent">02 /</span> DEL CÓDIGO AL ENCUENTRO</div><h2>Nos vemos fuera del editor.</h2></div><NuxtLink class="text-link" to="/agenda">Todos los encuentros <span aria-hidden="true">↗</span></NuxtLink></div><NuxtLink v-if="upcoming" to="/agenda" class="next-event"><div class="event-day"><strong>{{ eventDate(upcoming.date, { day: '2-digit' }) }}</strong><span>{{ eventDate(upcoming.date, { month: 'short', year: 'numeric' }) }}</span></div><div class="next-event-info"><span class="eyebrow accent">PRÓXIMO ENCUENTRO · {{ upcoming.topic }}</span><h3>{{ upcoming.title }}</h3><span class="muted">{{ upcoming.location }} <span class="separator">/</span> {{ eventDate(upcoming.date, { hour: '2-digit', minute: '2-digit' }) }} h</span></div><span class="circle-arrow" aria-hidden="true">↗</span></NuxtLink><div v-else class="empty-events"><h3>La conversación continúa.</h3><p>Consulta en Meetup las nuevas convocatorias y descubre lo que hemos compartido en encuentros anteriores.</p><a href="https://www.meetup.com/es-es/bilbo-dev/" class="text-link">Ver el grupo en Meetup ↗</a></div></section>
